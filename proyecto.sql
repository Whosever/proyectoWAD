...
CREATE TABLE municipios (
  ...
  activo int NOT null,
  foreign key (estado_id) references Estados(estado_id) on update cascade on delete cascade
)
...
create table usuarios(
	...
	tipo int not null,
	image varchar,
	foreign key (municipio_id) references municipios(municipio_id) on update cascade on delete cascade
);
...
create table Transacciones(
	...
	monto int not null,
	foreign key (origen) references usuarios(Usuario_id) on update cascade on delete cascade,
	foreign key (destino) references usuarios(Usuario_id) on update cascade on delete cascade
);

SELECT transacciones.transaction_id,
	transacciones.monto,
...
FROM transacciones
	INNER JOIN usuarios AS origen ON 
	 transacciones.origen = origen.usuario_id 
	INNER JOIN usuarios AS destino ON 
	 transacciones.destino = destino.usuario_id

INSERT INTO estados (estado_id, clave, nombre, abrev, activo) VALUES
...
INSERT INTO municipios (municipio_id, estado_id, clave, nombre, activo) VALUES
